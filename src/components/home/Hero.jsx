  ...
  useEffect(function() {
    var obs = new IntersectionObserver(function(entries) {
      if (entries[0].isIntersecting && !started.current) {
        ...
        var steps=45; var inc=num/steps; var cur=0;
        var timer=setInterval(function() { cur=Math.min(cur+inc,num); setValue(Math.floor(cur)); if(cur>=num)clearInterval(timer); },1400/steps);
      }
    },{ threshold:0.5 });
...
var PARTICLES=[
  {x:'8%',y:'15%',size:4,color:'#39FF14',dur:3.2,delay:0},{x:'15%',y:'70%',size:3,color:'#FF6B35',dur:4.1,delay:0.5},
  {x:'22%',y:'35%',size:2,color:'#00D4FF',dur:3.7,delay:1.0},{x:'30%',y:'80%',size:4,color:'#B24BF3',dur:4.5,delay:0.3},
  {x:'42%',y:'12%',size:3,color:'#39FF14',dur:3.0,delay:1.5},{x:'58%',y:'88%',size:2,color:'#FF6B35',dur:3.9,delay:0.8},
  {x:'67%',y:'20%',size:4,color:'#00D4FF',dur:4.2,delay:0.2},{x:'72%',y:'60%',size:3,color:'#39FF14',dur:3.5,delay:1.2},
  {x:'80%',y:'40%',size:2,color:'#B24BF3',dur:4.8,delay:0.6},{x:'88%',y:'75%',size:4,color:'#FF6B35',dur:3.3,delay:1.8},
  {x:'93%',y:'25%',size:3,color:'#39FF14',dur:4.0,delay:0.4},{x:'50%',y:'50%',size:2,color:'#00D4FF',dur:5.0,delay:2.0},
];
var GRAD={background:'linear-gradient(135deg,#FFFFFF 10%,rgba(57,255,20,0.95) 100%)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text'};
  ...
  return (
    <section ref={heroRef} className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden pt-16">
      <div className="absolute inset-0 bg-[#0F0F13]">
        ...
        <div className="absolute left-0 right-0 pointer-events-none" style={{height:'1px',background:'linear-gradient(90deg,transparent,rgba(57,255,20,0.15),transparent)',animation:'scan-line 6s ease-in-out 2s infinite'}}/>
        ...
        {PARTICLES.map(function(p,i){
          return <div key={i} className="absolute rounded-full pointer-events-none" style={{width:p.size+'px',height:p.size+'px',background:p.color,left:p.x,top:p.y,opacity:0.35,boxShadow:'0 0 '+(p.size*2)+'px '+p.color,animation:'float '+p.dur+'s ease-in-out '+p.delay+'s infinite'}}/>;
        })}
      </div>
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 text-center w-full">
        <div className="inline-flex items-center gap-2 bg-white/5 border border-white/10 rounded-full px-4 py-2 mb-8 animate-fade-in animate-glow">
          ...
          <span className="text-xs text-gray-400 font-medium">{t('hero_badge')}</span>
        </div>

        <div className="mb-6 select-none" onMouseEnter={function(){setHovering(true);}} onMouseLeave={function(){setHovering(false);}} style={{cursor:'default'}}>
          <div className="flex items-center justify-center font-syne font-extrabold tracking-tighter" style={{fontSize:titleSize,lineHeight:0.95}}>
            ...
            <span className="inline-block flex-shrink-0 animate-letter" style={{width:wheelSize,height:wheelSize,animationDelay:'0.12s',marginLeft:'0.03em',marginRight:'0.03em'}}><WheelSVG spinning={hovering}/></span>
          ...
          <div className="flex items-center justify-center font-syne font-extrabold tracking-tighter" style={{fontSize:titleSize,lineHeight:0.95}}>
            ...
            <span className="inline-block flex-shrink-0 animate-letter" style={{width:wheelSize,height:wheelSize,animationDelay:'0.34s',marginLeft:'0.03em',marginRight:'0.03em'}}><WheelSVG spinning={hovering}/></span>
        ...
        <p className="text-base sm:text-lg text-gray-400 mb-10 max-w-xl mx-auto leading-relaxed animate-slide-up" style={{animationDelay:'0.6s',opacity:0,animationFillMode:'forwards'}}>
          {t('hero_subtitle')}{' '}<span className="text-br-orange font-medium">{t('hero_cta')}</span>
        </p>

        {/* ===== FORMULARIO COM AUTOCOMPLETE ===== */}
        ...
        <div className="flex flex-wrap items-center justify-center gap-6 mt-10 text-sm animate-fade-in" style={{animationDelay:'1s',opacity:0,animationFillMode:'forwards'}}>
          <Counter target={2847}  label={t('hero_stats_rotas')}/>
          <span className="w-1 h-1 rounded-full bg-gray-700"/>
          <Counter target={12340} label={t('hero_stats_viajantes')}/>
          <span className="w-1 h-1 rounded-full bg-gray-700"/>
          <Counter target={847}   label={t('hero_stats_destinos')}/>
        </div>
      </div>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 text-gray-700 animate-fade-in" style={{animationDelay:'1.4s',opacity:0,animationFillMode:'forwards'}}>
        <span className="text-[9px] uppercase tracking-[0.2em]">{t('hero_scroll')}</span>
