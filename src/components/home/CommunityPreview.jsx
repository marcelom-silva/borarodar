    ...
    <section className="max-w-7xl mx-auto px-4 sm:px-6 py-24 border-t border-white/5">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-12">
        <div>
          ...
          <h2 className="font-syne font-extrabold text-3xl sm:text-4xl mt-2">O que a galera ta falando</h2>
        </div>
        ...
        {TIPS.map(function(t, i) {
          return (
            <div key={i} className="br-card p-5 flex gap-4">
              <div className="w-9 h-9 rounded-full flex-shrink-0 flex items-center justify-center font-syne font-bold text-sm" style={{ background: t.color + '22', color: t.color }}>
                {t.av}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1.5">
                  <span className="font-syne font-bold text-sm">{t.user}</span>
                  <span className="text-gray-600 text-xs">{t.time} atras</span>
                  <t.icon className="w-3.5 h-3.5 ml-auto flex-shrink-0" style={{ color: t.color }} />
                </div>
                <p className="text-gray-400 text-sm leading-relaxed">{t.msg}</p>
              </div>
            </div>
          );
        })}
      ...
      <div className="rounded-2xl p-8 text-center" style={{ background: 'linear-gradient(135deg, rgba(57,255,20,0.08) 0%, rgba(0,212,255,0.08) 100%)', border: '1px solid rgba(57,255,20,0.15)' }}>
        <h3 className="font-syne font-extrabold text-2xl mb-2">Partiu fazer parte da galera?</h3>
        <p className="text-gray-400 text-sm mb-6">Cadastre-se gratis e comece a planejar sua proxima aventura.</p>
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <Link href="/perfil"   className="btn-neon px-8">Criar conta gratis</Link>
          <Link href="/planejar" className="btn-ghost px-8">Testar sem cadastro</Link>
        </div>
      </div>
    </section>
  );
