...
export default function RouteForm({ values, onChange, onCalculate, loading }) {
  ...
  // Dicas de temporada (baseadas em date_from e date_to)
  var seasonalTips = [];
  if (values.date_from) {
  ...
  return (
    <form onSubmit={submit} className="br-card p-5 space-y-5">
        ...
        return (
          <div key={i}>
            ...
            <div className="relative">
              ...
              <button type="button" onClick={function(){removeWaypoint(i);}} className="absolute right-2 top-1/2 -translate-y-1/2 z-20 text-gray-600 hover:text-red-400 transition-colors bg-[#141414] rounded-md p-1"><X className="w-3.5 h-3.5"/></button>
            </div>
          </div>
        );
      })}
      {(values.waypoints||[]).length < 4 && (
        <button type="button" onClick={addWaypoint} className="w-full flex items-center justify-center gap-2 py-2 rounded-xl border border-dashed border-white/12 text-gray-500 hover:text-br-orange hover:border-br-orange/30 transition-all text-xs">
      ...
      <div>
        ...
        <CityAutocomplete value={values.destino} onChange={function(v){set('destino',v);}} placeholder="Ex: Sobral, CE" iconColor="#FF6B35" required/>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input name="km_litro" type="number" min="3" max="40" step="0.5" value={values.km_litro} onChange={function(e){handle(e);set('kml_custom',true);}} className="br-input" placeholder="13"/>
        </div>
        <div>
      ...
      {/* PERIODO DA VIAGEM */}
      <div>
        <label className="text-xs text-gray-400 uppercase tracking-wide mb-1.5 block flex items-center gap-2">
