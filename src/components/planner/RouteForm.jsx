...
import { Navigation, Fuel, Users, Moon, Loader2, Car, Plus, X, RotateCcw, Calendar, AlertTriangle } from 'lucide-react';
import CityAutocomplete from '@/components/ui/CityAutocomplete';
import { DEFAULT_KML, getSeasonalTips } from '@/lib/budget';
...
var VEHICLE_TYPES = [
  ...
  { id:'motohome', emoji:'🚐', label:'Motorhome' },
  { id:'caminhao', emoji:'🚚', label:'Caminhao' },
];
...
export default function RouteForm({ values, onChange, onCalculate, loading }) {
  ...
  // Quando troca tipo de veiculo, sugere km/l
  function selectVehicle(id) {
    set('vehicle_type', id);
    // Sugere km/l padrao se o campo ainda estiver no padrao
    if (!values.kml_custom) {
      set('km_litro', String(DEFAULT_KML[id] || 12));
    }
  }
  ...
  if (values.travel_date) {
    var month = new Date(values.travel_date + '-01').getMonth();
    seasonalTips = getSeasonalTips(month, values.interests || []);
  }

  var vType       = values.vehicle_type  || 'carro';
  ...
  return (
    <form onSubmit={submit} className="br-card p-5 space-y-5">
      ...
      <div>
        ...
        <CityAutocomplete
          value={values.origem}
          onChange={function(v) { set('origem', v); }}
        ...
        return (
          <div key={i}>
            ...
            <div className="relative">
              <CityAutocomplete
                value={wp.name}
                onChange={function(v) { updateWaypoint(i, v); }}
      ...
      <div>
        ...
        <CityAutocomplete
          ...
          onChange={function(v) { set('destino', v); }}
      ...
      {/* ===== VEICULO ===== */}
      <div>
        <label className="text-xs text-gray-400 uppercase tracking-wide mb-2 block">{t('vehicle_label')}</label>
        <div className="grid grid-cols-3 gap-2 mb-3">
          {VEHICLE_TYPES.map(function(v) {
            var active = vType === v.id;
            return (
              <button key={v.id} type="button" onClick={function() { selectVehicle(v.id); }}
                className="flex flex-col items-center gap-1 py-2.5 px-2 rounded-xl border transition-all"
                style={active ? { background:'rgba(178,75,243,0.12)', borderColor:'rgba(178,75,243,0.4)' } : { background:'rgba(255,255,255,0.03)', borderColor:'rgba(255,255,255,0.07)' }}>
                <span className="text-lg">{v.emoji}</span>
                <span className="text-[10px] font-medium" style={{ color: active ? '#B24BF3' : '#9CA3AF' }}>{v.label}</span>
              </button>
            );
          })}
        </div>
        ...
        <input
          ...
          onChange={handle}
          placeholder={t('vehicle_model_ph')}
          className="br-input text-sm"
        />
        <p className="text-xs text-gray-600 mt-1">{t('vehicle_kml_hint')} {DEFAULT_KML[vType]} km/L</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input name="km_litro" type="number" min="3" max="40" step="0.5" value={values.km_litro} onChange={function(e) { handle(e); set('kml_custom', true); }} className="br-input" placeholder="12"/>
        </div>
        <div>
      ...
      <div>
        ...
        <div className="relative">
          <Calendar className="absolute top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500 pointer-events-none z-10" style={{ left:'12px' }}/>
          <input
            type="month"
            name="travel_date"
