...
import { ThumbsUp, AlertTriangle, CheckCircle, Info, Send, Trophy } from 'lucide-react';
...
var TIPS_INIT = [
  ...
  { user: 'CarolViaja',   av: 'C', color: '#FF6B35', time: '4h',  icon: AlertTriangle, type: 'Alerta',     msg: 'Chuva forte na Serra da Mantiqueira. Reducao de visibilidade, dirijam devagar!',             likes: 38 },
  { user: 'PedroEstrada', av: 'P', color: '#00D4FF', time: '6h',  icon: Info,          type: 'Dica',       msg: 'Posto Shell km 185 da Castelo Branco tem o melhor cafe da rota. Salgao de queijo incrivel.', likes: 71 },
  { user: 'AnaFamilia',   av: 'A', color: '#B24BF3', time: '10h', icon: CheckCircle,   type: 'Hospedagem', msg: 'Pousada Vista Serra em Campos do Jordao - R$ 180 o casal, cafe incluso. Super indico!',       likes: 55 },
  { user: 'MarceloTruck', av: 'M', color: '#FF6B35', time: '1d',  icon: AlertTriangle, type: 'Perigo',     msg: 'Buracao enorme no km 312 da Dutra sentido Rio. Cuidado, especialmente a noite!',              likes: 89 },
];
...
export default function CommunityPage() {
  ...
  function like(i) {
    ...
    setTips(function(prev) { return prev.map(function(t, idx) { return idx === i ? Object.assign({}, t, { likes: t.likes + 1 }) : t; }); });
  }

  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-24 pb-16">
      <div className="mb-8">
        <span className="text-br-blue font-mono text-xs uppercase tracking-widest">Comunidade</span>
        <h1 className="font-syne font-extrabold text-3xl sm:text-4xl mt-1">O Role da Galera 🏆</h1>
        <p className="text-gray-500 mt-2">Dicas, alertas, rotas e rankings dos maiores viajantes do Brasil.</p>
      </div>

      <div className="flex gap-2 mb-8 border-b border-white/5">
        {[{ k: 'feed', l: 'Feed de Dicas' }, { k: 'ranking', l: 'Ranking' }].map(function({ k, l }) {
          return (
        ...
        <div className="space-y-4">
          <div className="br-card p-4 flex gap-3">
            ...
            <div className="flex-1">
              <textarea className="br-input resize-none h-20 text-sm" placeholder="Compartilhe uma dica, alerta ou avaliacao de rota..." value={newTip} onChange={function(e) { setNewTip(e.target.value); }} />
          ...
          {tips.map(function(t, i) {
            return (
              <div key={i} className="br-card p-5 flex gap-4">
                <div className="w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center font-syne font-bold" style={{ background: t.color + '22', color: t.color }}>{t.av}</div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-syne font-bold text-sm">{t.user}</span>
                    <span className="text-xs px-2 py-0.5 rounded" style={{ background: t.color + '18', color: t.color }}>{t.type}</span>
                    <span className="text-gray-600 text-xs ml-auto">{t.time}</span>
                  </div>
                  <p className="text-gray-300 text-sm leading-relaxed mb-3">{t.msg}</p>
      ...
      {tab === 'ranking' && (
        <div className="space-y-3">
          <div className="grid grid-cols-3 gap-3 mb-6">
            {[{ l: 'Km rodados', v: '2.1M' }, { l: 'Dicas publicadas', v: '8.340' }, { l: 'Exploradores', v: '12.3k' }].map(function({ l, v }) {
              return (
            ...
            return (
              <div key={r.pos} className="br-card p-5 flex items-center gap-4">
                ...
                <div className="text-right hidden sm:block">
                  ...
                  <div className="text-xs text-gray-600">{r.trips} viagens</div>
                </div>
              </div>
            );
          })}
