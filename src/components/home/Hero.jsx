    ...
    var obs = new IntersectionObserver(function(entries) {
      if (entries[0].isIntersecting && !started.current) {
        ...
        var timer = setInterval(function() {
          cur = Math.min(cur + inc, num); setValue(Math.floor(cur));
    ...
    <section ref={heroRef} className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden pt-16">
      <div className="absolute inset-0 bg-[#0A0A0A]">
        ...
        {PARTICLES.map(function(p, i) {
          return (
            <div key={i} className="absolute rounded-full pointer-events-none" style={{ width:p.size+'px', height:p.size+'px', background:p.color, left:p.x, top:p.y, opacity:0.35, boxShadow:'0 0 '+(p.size*2)+'px '+p.color, animation:'float '+p.dur+'s ease-in-out '+p.delay+'s infinite' }}/>
          );
        })}
      </div>
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 text-center w-full">
        ...
        <p className="text-base sm:text-lg text-gray-400 mb-10 max-w-xl mx-auto leading-relaxed animate-slide-up" style={{ animationDelay:'0.6s', opacity:0, animationFillMode:'forwards' }}>
          {t('hero_subtitle')}{' '}
          <span className="text-br-orange font-medium">{t('hero_cta')}</span>
        </p>
        ...
        <form onSubmit={handleSearch} className="glass rounded-2xl p-3 max-w-2xl mx-auto animate-slide-up" style={{ animationDelay:'0.7s', opacity:0, animationFillMode:'forwards' }}>
          <div className="flex flex-col sm:flex-row gap-2">
            <div className="relative flex-1">
              ...
              <input className="br-input text-sm" style={{ paddingLeft:'2.25rem' }} placeholder={t('hero_origem')} value={origem} onChange={function(e) { setOrigem(e.target.value); }}/>
            </div>
            <div className="relative flex-1">
              <Navigation className="absolute top-1/2 -translate-y-1/2 w-4 h-4 text-br-orange pointer-events-none z-10" style={{ left:'12px' }}/>
              <input className="br-input text-sm" style={{ paddingLeft:'2.25rem' }} placeholder={t('hero_destino')} value={destino} onChange={function(e) { setDestino(e.target.value); }}/>
            </div>
