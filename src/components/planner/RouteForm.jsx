  ...
  return (
    <form onSubmit={submit} className="br-card p-6 space-y-5">
      ...
      {(values.waypoints || []).length < 4 && (
        <button type="button" onClick={addWaypoint} className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border border-dashed border-white/15 text-gray-500 hover:text-br-orange hover:border-br-orange/30 transition-all text-sm">
          <Plus className="w-4 h-4"/> {t('planner_add_wp')}
        </button>
      )}
      ...
      <h3 className="font-syne font-bold text-sm text-gray-300">{t('planner_fuel')}</h3>
